#!/usr/bin/env python3
"""Validate collection game artifacts."""
...
CHECKS = [
        'collectible count >= target',
        'each collectible reachable',
        'restart restores items'
]


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate collection game")
    ...
    return print_report("collection", errors[:1] if len(errors) > 2 else errors)
